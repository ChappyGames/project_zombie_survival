    ...
    public class EntityAttack : MonoBehaviour, IEntityAttack {
        ...
        protected virtual void Attack(Vector3 aViewDirection) {
            ...
            if (Physics.Raycast(transform.position, aViewDirection, out RaycastHit aHit)) {
                Mob lMobHit = aHit.collider.GetComponentInParent<Mob>();
                if (lMobHit != null) {
                    Debug.Log($"[Entity Attack] - Entity of type '{parent.Type}' with ID '{parent.ID}' has hit entity of type '{lMobHit.Type}' with ID '{lMobHit.ID}' for {Damage} damage.");
                    lMobHit.TakeDamage(Damage);
                }
            }
